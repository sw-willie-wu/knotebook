...
- A rejected collaboration handshake no longer tells people they lost access to a note when they did not. The server had one rejection reason for three different situations, and the client turned all of them into "You no longer have access to this note", closed the note and returned to the list. So a rotated `APP_SECRET`, a clock skew large enough to expire a token in flight, or a disabled account each produced that notice for someone whose permissions were untouched — and a note that had finished deleting produced it too, instead of saying the note was gone. The server now distinguishes them, the client only treats a real access decision as one, and everything else retries the handshake on the same widening delay used for token failures. A revoked share still converges on the same notice it always did (#35).
...
- Every rejected collaboration handshake now leaves a line in the server log — `noteId`, `userId`, and which of the five causes it was — and an unexpected error inside the handshake is logged with its stack trace instead of being flattened into a generic rejection. Since a rejection now navigates the person away rather than leaving them stuck on "Connecting", the report that reaches the maintainer is "I got kicked out", with nothing on either side to say why. See [Troubleshooting](docs/self-hosting.md#troubleshooting) (#37).
