...
- An embedded image, video, audio or file URL is now checked again when it is rendered, not only when it is typed into the embed field. Note content is a collaborative Yjs document, so any collaborator with edit access can write block properties directly and bypass the input check entirely; a `javascript:` URL written that way reached every other reader unfiltered. Uploaded files keep working — the render-side check deliberately accepts the relative URLs uploads produce (#12).
