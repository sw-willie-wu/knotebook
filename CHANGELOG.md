...
- A refused collaboration connection now leaves a line in the server log — which note, which account, which of four causes, and whether it was refused while connecting or closed after a permission re-check — and an unexpected error inside the handshake is logged with its stack trace instead of being flattened into a generic rejection. Since a refusal now navigates the person away rather than leaving them stuck on "Connecting", the report that reaches the maintainer is "I got kicked out", with nothing on either side to say why. The lines are capped per account, and refusals decided before the token identifies anyone are logged at `debug`, so neither an anonymous socket nor a single account can drive the log volume. See [Troubleshooting](docs/self-hosting.md#troubleshooting) (#37).
