...
- Every rejected collaboration handshake now leaves a line in the server log — `noteId`, `userId`, and which cause it was — and an unexpected error inside the handshake is logged with its stack trace instead of being flattened into a generic rejection. Since a rejection now navigates the person away rather than leaving them stuck on "Connecting", the report that reaches the maintainer is "I got kicked out", with nothing on either side to say why. Handshakes rejected before the token identifies anyone are logged at `debug` instead, so an anonymous socket cannot drive the log volume. See [Troubleshooting](docs/self-hosting.md#troubleshooting) (#37).
