...
| `/api/notes/:id/public-link` | GET | Auth (owner) | 200 `{token: string | null}`, 403 `forbidden`, 404 `not_found` |
| `/api/notes/:id/public-link` | PUT | Auth (owner) | 200 `{token}` (regenerates on every call), 403 `forbidden`, 404 `not_found`, 429 `too_many_requests` |
| `/api/notes/:id/public-link` | DELETE | Auth (owner) | 204, 403 `forbidden`, 404 `not_found`, 429 `too_many_requests` |
| `/api/public/notes/:token` | GET | none | 200 `{title, ydoc: base64}` with `Cache-Control: no-store` (the note's title and its content snapshot as a Yjs update; deliberately no note id and no timestamp — see [Sharing](./sharing.md)), 404 `not_found` (byte-identical for a revoked, regenerated, never-issued, or malformed token — the response never distinguishes "no such note" from "not shared publicly"), 429 `too_many_requests` (two buckets: per-IP for tokens that resolve to nothing, per-IP-and-token for reads of a live link) |
