...
- **Treat a token like a password.** A `notes:write` token can **rewrite the content of any note you can edit** — not just create new ones — and those writes go straight into the live document everyone is looking at. A leaked one is a credential that acts as you, on your notes. Nothing is destroyed silently: a successful API write that changes a note's content is recorded, listed by `GET /api/notes/:id/edits`, and can be reverted while the note has not moved on — but a note keeps at most 100 entries. Prefer `notes:read` for anything that only needs to read, and don't paste a token into shared config, logs, or chat.
