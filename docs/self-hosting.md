...
- **Someone reports being kicked out of a note ("You no longer have access"), or a tab that sits on "Connecting" and never recovers.** Rejected collaboration handshakes are logged by the `app` container as `collab 握手被拒`, with structured fields `noteId`, `userId`, `cause` and `reason` — `docker compose logs app | grep '"cause"'` shows them. Four `cause` values appear at the default log level, and all four carry the `userId` of the account that was refused:
  - `no-role` — the account genuinely has no role on that note. **This is the only cause that means what the "You no longer have access" notice says**, and the only one that ejects the person.
  - `note-deleting` — the note was deleted, or is being deleted right now. The tab shows "note deleted" and returns to the list. The window this covers is bounded (see `DELETING_GATE_TTL_MS`, currently two minutes); a tab that reconnects later than that gets `no-role` instead, because the server deliberately cannot tell "deleted" from "not yours" once the record is gone.
  - `session-revoked` — the account was disabled, or its sessions were invalidated (a password change). This is **not** an access decision about the note: the tab fetches a new token within seconds, that request 401s, and the tab signs itself out. If you expected that account to still work, look at `users.disabled_at` and `users.token_version`.
  - `server-error` — the handshake hit an unexpected error, logged at error level with the stack trace and the same `cause` field. **This is the one that leaves a tab retrying on "Connecting" indefinitely**, so if that is the complaint, grep for this first; the usual culprit is the database being unreachable.

  A fifth cause, `bad-token`, is logged at `debug` and so is invisible at the default level. It means the handshake presented no token, a token that failed to verify, or a token bound to a different note. It is deliberately kept off the default level because `/collab` accepts WebSocket upgrades without a session and the server does not close the socket on a failed handshake — logging it would let any anonymous client decide how much disk your logs consume. A tab hitting it recovers on its own.
