...
export interface AppConfig { databaseUrl: string; appSecret: string; publicUrl: URL; cookieSecure: boolean; bootstrapAdminEmail?: string }
export function loadConfig(env: NodeJS.ProcessEnv): AppConfig {
  ...
  if (publicUrl.protocol === "http:" && !["localhost", "127.0.0.1", "[::1]"].includes(publicUrl.hostname))
    throw new Error("PUBLIC_URL 為 http 時僅允許 localhost；對外請用 https（自備 reverse proxy + TLS）");
  return { databaseUrl: r.data.DATABASE_URL, appSecret: r.data.APP_SECRET, publicUrl,
           cookieSecure: publicUrl.protocol === "https:", bootstrapAdminEmail: r.data.BOOTSTRAP_ADMIN_EMAIL };
}
